...
kable(table2 %>%
  ...
  kableExtra::group_rows(table2Index$Header[14], table2Index$Start[14], table2Index$End[14])
```
...
table1Testing <- bind_rows(
  ...
  mutate(Header = str_remove(Header, " \\[Range\\], Median \\[IQR\\]"))

table1Chisq <- table1Testing %>%
  filter(Header != "Monthly Income") %>%
  mutate_at(vars(-Header, -Variable), list(as.integer))

table1Chisq_vars <- unique(table1Chisq$Header)
...
tableAnova <- bind_rows(
  table1Testing %>%
    filter(Header == "Monthly Income"),
  table2 %>%
    filter(str_detect(Header, "\\[Range\\]")) %>%
    select(-Overall_2) %>%
    set_names(str_remove(colnames(.), " \\(.*\\)")) %>%
    rename(Overall = Overall_1)
) %>%
  filter(!is.na(Overall))
