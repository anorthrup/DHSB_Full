...
table1 <- bind_rows(
  ...
  table_OneFactor(demo, varString = "Age Groups",
                  varRelevel = c("Under 18 Years", "18-24 Years")),
  #Ethnicity/Race
  ...
  table_OneFactor(demo, varString = "Education", 
                  varRelevel = c("High school, equivalent or less",
                                 "Some post-K12")),
  #Employment
  table_ManyBinary(demo, header = "Employment",
                   variables = c("EMPLOYA" = "Student",
                                 "EMPLOYB" = "Full-time employed",
                                 "EMPLOYC" = "Part-time employed",
                                 "EMPLOYD" = "Disabled",
                                 "EMPLOYE_RC" = "Unemployed")),
  #Income
  bind_rows(
    demo %>%
...
kable(table1 %>%
  ...
  group_rows("Age Group, N (%)", 2, 5) %>%
  group_rows("Ethnicity and Race, N (%)", 6, 10) %>%
  group_rows("Sex at Birth, N (%)", 11, 12) %>%
  group_rows("Gender, N (%)", 13, 16) %>%
  group_rows("Sexual Orientation, N (%)", 17, 20) %>%
  group_rows("Education, N (%)", 21, 24) %>%
  group_rows("Employment, N (%)", 25, 29) %>%
  group_rows("Income, Median [IQR]", 30, 30) %>%
  group_rows("Residence, Last 7 Days, N (%)", 31, 37) %>%
  group_rows("Incarceration, N (%)", 38, 38)
```
