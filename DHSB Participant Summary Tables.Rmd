...
demo <- acasi2 %>%
  ...
  mutate(`Age Groups` = as.factor(case_when(AGE < 25 ~ "18-24 Years",
         ...
         ViralSupp = fct_recode(as.factor(ViralSupp),
                                "Not suppressed" = "0",
                                "Suppressed"     = "1"),
         MONEY_RC_Log = log(MONEY_RC)) %>%
...
table1 <- bind_rows(
  ...
  #Income
  table_Continuous(demo, variable = "MONEY", stat = "median", 
...
table2 <- bind_rows(
  ...
  table_ManyBinary(demo, header = "Substance Use", n = 0,
                   keep = c("DRUG1LA", "DRUG1LB", "DRUG1LC", "DRUG_RC_None",
                            "INJECTL"),
                   variables = c("DRUG1LA" = "Alcohol",
                                 ...
                                 "DRUG2LI" = "Other non-injected drugs",
                                 "DRUG_RC_None" = "None",
...
kable(table2 %>%
  ...
  group_rows("Social Support, Mean (SD)", 20, 20) %>%
  group_rows("HIV-Related Stigma, Mean (SD)", 21, 21) %>%
  group_rows("Disclosure of HIV Status, N (%)", 22, 31) %>%
  group_rows("Physical and Mental Health, Mean (SD)", 32, 33) %>%
  group_rows("Substance Use, N (%)", 34, 40) %>%
  group_rows("Media and Technology Use and Attitudes Subscales", 41, 48)
```
...
bind_rows(
  ...
  chisq.dhsb(table1Chisq, "Education", 
             c("   High school, equivalent or less",
               "   Some post-K12",
               "   Associate, trade cert, or higher")),
  chisq.dhsb(table1Chisq, "Employment Status", 
             c("   Employed/Student",
               "   Unemployed/Disabled")),
  chisq.dhsb(table1Chisq, "Residence, Last 7 Days",
             c("   Rented house/apt/flat",
               "   Other's house/apt/flat",
               "   Hotel/Shelter/Rehab",
               "   Hospital/Medical facility",
               "   Jail/Juvenile detention"))
) %>%
  mutate(Warning = c("No", "Yes", "Yes", "Yes", "Yes", "No", "Yes"))
bind_rows(
