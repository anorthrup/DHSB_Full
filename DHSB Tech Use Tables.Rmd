...
table_ManyBinary_Lump <- function (x, n, header, variables, keep = NULL) {
  ...
  full_join( #Create two data frames and join them together: summary of overall, summary by site
    ...
    mutate(Variable = c(
      ...
      "Other", 
      variables[which(names(variables) == keep)])) %>%
    #Add totals row
      {
        bind_rows( #Bind total participants row to previous table
          { #Calculate total participants
            bind_cols( #Calculate totals for overall and by site separately
              summarize(x, Overall = paste0(n(), " (100.0%)")), #Overall
              group_by(x, SITE_RC) %>% #By site
...
table_OneFactor_Lump <- function (x, n, varString, header = varString, 
  ...
  y <- x %>% 
    select(varString) %>%
    map(~which(. == "Refuse to answer")) %>%
    unlist()
  if (any(y > 0)) {
    print("The following rows contain response 'Refuse to answer':")
    print(y)
  }
  freqLevels <- x %>%
    select(varString) %>%
  ...
  x %>%
    ...
    mutate_at(vars(one_of(varString)), funs(fct_recode(., !!!levRecode))) %>%
    mutate_at(vars(one_of(varString)), funs(fct_relevel(., keep, after = length(freqLevels)))) %>%
    ...
    spread(Site, Frequency) %>%
    {
      bind_rows( #Create totals row and bind to previous summary table
        x %>%
        {
          bind_cols( #Must summarize overall totals and totals by site separately
    ...
    mutate(Variable = fct_reorder(as.factor(Variable), 
           ...
           Variable = fct_relevel(Variable, "Other", 
                                  after = length(levels(Variable)) - 1)) %>%
    arrange(Variable) %>%
