...
table_OneFactor_Lump <- function (x, n, varString, header = varString, 
  ...
  y <- x %>% 
    select(varString) %>%
    map(~which(. == "Refuse to answer" | is.na(.))) %>%
    unlist()
  if (any(y > 0)) {
    print("The following rows contain contain 'Refuse to answer' or 'NA':")
    print(y)
    nonbinary <- x %>%
      select(SITE_RC, varName) %>%
      mutate_at(
        vars(one_of(varName)), 
        funs(replace(., which(. == "Refuse to answer"), NA))) %>%
      mutate(NumNA = if_else(rowSums(is.na(.)) > 0, 1, 0)) %>%
        {
          bind_cols(
            summarize(., Overall = as.character(sum(NumNA))),
      ...
      mutate(Variable = "Refused/Missing")
  } else {
    nonbinary <- NULL
  }
