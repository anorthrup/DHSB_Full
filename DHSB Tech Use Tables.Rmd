...
tableTech <- function (x, n, header, varNames, varStrings) { 
  ##varStings must be in same order as varNames
  ...
  full_join(
    ...
    mutate(Variable = c(
      varStrings[map_int(Variable[1:n], ~which(varNames %in% .))],
      "Other")) %>%
    #Add header row
      {
        bind_rows(
...
table3 <- bind_rows(
  tableTech(tech, n = 3, header = "Device Use",
            varNames = c("S56_1A", "S56_1B", "S56_1C", "S56_1D", "S56_1E", 
                         "S56_1F", "S56_1G"),
